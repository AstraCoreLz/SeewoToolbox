...
public class MainViewModel : ViewModelBase
{
    ...
    private async Task InstallClassIslandAsync()
    {
        ...
        var isArm = arch == Architecture.Arm64;
        var archLabel = isArm ? "ARM64" : "X64";

        var choice = await ShowChoiceDialogAsync(
            $"安装 ClassIsland ({archLabel})",
            $"已检测到当前系统架构：{archLabel}\n\nClassIsland 是一个 Windows 桌面美化小组件工具。\n\n请选择版本（有冰点还原的设备建议选含运行时版本）：",
            $"{archLabel} 含运行时（推荐）",
            $"{archLabel} 便携版");
        ...
        string url, version;
        var baseDir = "https://get.classisland.tech/d/ClassIsland-Ningbo-S3/classisland/distribution-v2/2.1/2.1.0.1";
        var archStr = isArm ? "arm64" : "x64";

        switch (choice)
        {
            case 1:
                url = $"{baseDir}/ClassIsland_app_windows_{archStr}_selfContained_folder.zip";
                version = $"{archLabel} 含运行时";
                break;
            case 2:
                url = $"{baseDir}/ClassIsland_app_windows_{archStr}_full_folder.zip";
                version = $"{archLabel} 便携版";
                break;
            default:
                return;
        }
    ...
    private static async Task<int> ShowChoiceDialogAsync(string title, string message, string button1, string button2)
    {
        var mainWindow = (Application.Current?.ApplicationLifetime as Avalonia.Controls.ApplicationLifetimes.IClassicDesktopStyleApplicationLifetime)?.MainWindow;
        if (mainWindow == null) return 0;
        ...
        await Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(() =>
        {
            var dialog = CreateDialog(title, 400, 360);
            var panel = CreateDialogContent();
            ...
            var btn2Control = CreateGhostButton(button2);
            btn2Control.Margin = new Thickness(0, 8, 0, 0);
            btn2Control.Click += (_, _) => { tcs.TrySetResult(2); dialog.Close(); };
